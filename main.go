...
type Superhero struct { //Recurso superheroe
...
var superheroes = []Superhero{ //Super heroes disponibles en memoria
	{
		Name: "Spider-Man",
		Biography: struct {
			FullName string `json:"fullName"`
...
func getSuperhero(w http.ResponseWriter, r *http.Request) { //Handler
	heroName := strings.ToLower(r.URL.Query().Get("hero"))
	for _, hero := range superheroes {
		if strings.ToLower(hero.Name) == heroName {
